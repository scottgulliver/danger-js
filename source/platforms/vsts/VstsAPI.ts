...
import { VstsPRDSL, VstsUser } from "../../dsl/VstsDSL"
...
import { RepoMetaData } from "../../dsl/VstsDSL"
...
export class VstsAPI {
  ...
  getPullRequestInfo = async (): Promise<VstsPRDSL> => {
    throw "Not implemented: getPullRequestInfo"
  }
  /*  if (this.pr) {
      return this.pr
    }
  ...
  /*getUserInfo = async (): Promise<VstsUser> => {
  ...
  private api = (path: string, headers: any = {}, body: any = {}, method: string, suppressErrors?: boolean) => {
    if (this.token && !headers["Authorization"]) {
      headers["Authorization"] = `basic ${this.token}` //todo: accept a non-base64 string
    }

    const containsBase = path.startsWith("http")
    const baseUrl = process.env["DANGER_VSTS_API_BASE_URL"]
